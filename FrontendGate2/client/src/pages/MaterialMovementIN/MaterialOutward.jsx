...
export default function MaterialOutward() {
  ...
  const onSubmit = async (ev) => {
    ...
    try {
      const payload = { ...form };
      // Ensure WeightDocNumber is included
  ...
  // Fetch next weight doc number on mount
  useEffect(() => {
    const fetchNextNumber = async () => {
      try {
        const year = new Date().getFullYear().toString();
        // 3-series (253xxxxxxx)
        const resp = await fetchNextWeightDocNumber(year, 3);
        const nextNumber = resp?.data?.next || resp?.data?.value || resp?.data || '';
        setForm(prev => ({ ...prev, WeightDocNumber: String(nextNumber || '') }));
      } catch (e) {
        console.error('Failed to fetch next weight number', e);
      }
    };
    fetchNextNumber();
  }, []);
        ...
        <section className="form-section">
          ...
          <div className="grid-2-cols">
            <div className="form-group">
              ...
              <input type="text" name="WeightDocNumber" value={form.WeightDocNumber} className="form-input" readOnly />
            </div>
