...
export default function MaterialInward() {
  ...
  const onSubmit = async (ev) => {
    ...
    setLoading(true);
    try {
      // build payload in server-friendly shape
      const payload = { ...form };

      // Ensure WeightDocNumber exists before proceeding
      if (!payload.WeightDocNumber || payload.WeightDocNumber.trim() === '') {
        setError('Weight Document Number is required. Please refresh the page.');
        setLoading(false);
        return;
      }
      ...
      // SAP might not expect these fields for Material Inward
      delete payload.InwardTime;
      delete payload.OutwardTime;
      
      // Alternatively, if you need to send them, format as SAP duration PT16H42M16S
      // But based on the error, it seems SAP doesn't want these fields for weight bridge

      // SAP created timestamp — send iso
      payload.SAP_CreatedDateTime = nowIso();

      // Remove empty string fields to avoid conversion errors
      ...
      // Set result as an object with the actual data
      setResult({
      ...
      if (resp) {
        // Check if backend sent a simplified error string
  ...
  // Add useEffect to fetch WeightDocNumber on mount
  useEffect(() => {
    fetchNextWeightNumber();
  }, []);
